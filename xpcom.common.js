...
function common_xpcom(){
    ...
    this.get_server_params = function(){
        ...
        this.ajax_loader = '../../server/load.php';
    ...
    this.get_modules = function(){
        ...
        this.load(
            ...
            function(result){
                ...
                if (result.template == 'blue_purple'){

                    if (single_module.length == 0) {
                        _debug('redirect to the new launcher');
                        window.stop();
                        document.body.hide();
                        _debug('../blue_purple/launcher/?profile=' + encodeURIComponent('../../server/api/launcher_profile.php?uid=' + this.user['id'] + '&language='+this.stb_lang));
                        window.location = '../blue_purple/launcher/?profile=' + encodeURIComponent('../../server/api/launcher_profile.php?uid=' + this.user['id'] + '&language='+this.stb_lang);

                        return;
                    }
                }
		
		result.template = "smart_launcher";
console.log("output" + result.template);
    ...
    this.load = function(params, var_args){
        ...
        try{
            ...
            if (method == 'POST'){
                ...
                req.open("GET",  '../../server/load.php?' + this.params_to_query(params), true);
            }
    ...
    this.check_update = function(params){
        ...
        if (typeof params == 'object' &&
            ...
            if ((this.num_banks == 2 || ['MAG322', 'MAG324', 'MAG420'].indexOf(this.type) >= 0 ) && params.update_type == 'http_update'){
                try{
    ...
    this.user_init = function(user_data){
        ...
        if (['MAG200', 'MAG245','MAG245D', 'MAG250', 'MAG254', 'MAG255', 'MAG270', 'MAG275', 'WR320', 'MAG322', 'MAG324', 'MAG420', 'IP_STB_HD'].indexOf(this.type) === -1 && !_GET['debug_key']){
            ...
            if (!match || match.length != 2 || player_version < 1382){
                ...
                if (['MAG200', 'MAG245','MAG245D', 'MAG250', 'MAG254', 'MAG255', 'MAG270', 'MAG275', 'WR320', 'MAG322', 'MAG324', 'MAG420', 'IP_STB_HD'].indexOf(this.type) >= 0 || this.type.indexOf('AuraHD') != -1){
        ...
        if (this.user['status'] == 2){
            ...
            if (this.user['template'] == 'blue_purple' && single_module.length == 0){
                _debug('redirect to the new launcher');
                window.stop();
                document.body.hide();
                _debug('../blue_purple/launcher/?profile=' + encodeURIComponent('../../server/api/launcher_profile.php?uid=' + this.user['id'] + '&language='+this.stb_lang));
